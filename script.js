...
const TOOLS = {
  ...
  crt: {
    ...
    settings: () => `
      ...
      <div class="setting-group">
        <label>Barrel Distortion <span id="barrelVal">30</span>%</label>
        <input type="range" id="crtBarrel" min="0" max="100" value="30"
          oninput="document.getElementById('barrelVal').textContent=this.value" />
      </div>
      <div class="setting-group">
...
const MONOCHROMATIC_PALETTES = new Set(['gameboy', 'grayscale']);
...
function runCRT() {
  ...
  const barrel     = (parseInt(document.getElementById('crtBarrel').value)  || 0) / 100;
  ...
  if (barrel > 0) {
    applyBarrelDistortion(sCtx, dCtx, W, H, barrel * 0.5);
  } else {
...
function applyBarrelDistortion(srcCtx, dstCtx, W, H, strength) {
  const srcData = srcCtx.getImageData(0, 0, W, H);
  const dstData = dstCtx.createImageData(W, H);
  const src = srcData.data;
  const dst = dstData.data;

  // True CRT barrel distortion:
  // For each destination pixel, we find where in the SOURCE it came from.
  // We map distorted coords -> undistorted source coords using inverse barrel.
  // Barrel: dst = src * (1 + k * r²)  — edges bow outward
  // To fill the output, we walk dst pixels and sample from src using:
  //   srcUV = dstUV / (1 + k * |dstUV|²)  — this is the inverse (pincushion lookup)
  // which correctly produces barrel bowing on the output.

  const k = strength * 1.2; // strength multiplier — visible at lower slider values
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      // Inverse barrel lookup: where did this output pixel come from in source?
      const factor = 1 + k * r2;
      ...
      if (srcX >= 0 && srcX < W && srcY >= 0 && srcY < H) {
        ...
        // Black border outside the distorted area — authentic CRT look
